...
function dhtmlXGridObject(id) {
    ...
    //this.objBox.style.overflowX = "hidden";//xlj add
    ...
    this.setRowTextStyle2 = function(row_id, styleString) {//qgy new create for inp order gird
        var r = this.getRowById(row_id);
        if (!r)
            return;
        for (var i = 0; i < r.childNodes.length; i++) {
        	if(i==3) continue;
        	if(i==10 && typeof (r.childNodes[10].innerText)!="undefined" && r.childNodes[10].innerText.indexOf("-")==0) continue;
            var pfix = r.childNodes[i]._attrs["style"] || "";
            if ((this._hrrar) && (this._hrrar[i]))
                pfix = "display:none;";
            if (_isIE)
    ...
    if ((!_isOpera) || (_OperaRv > 8.5)) {
    	//xlj:Cancel colResize ---> delete here
    	/*
