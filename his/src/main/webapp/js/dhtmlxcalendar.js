function dhtmlXCalendarObject(inps, skin) {
	...
	this.setPosition = function(x, y) {
		if (x == "right" || x == "bottom" || x == "top" || x == "left") {//new add top、left, qgy
			this.pos = x;
			return;
		}
		if (!this._hasParent) {
	...
	this._show = function(inpId, autoHide) {
		...
		if (!inpId) {
			...
			}else if (this.pos == "top") {//new add top, qgy
				this.base.style.left = this._getLeft(this.i[inpId])+"px";
				this.base.style.top = this._getTop(this.i[inpId])-$(this.base).height()+"px";
			}else if (this.pos == "left") {//new add left, qgy
				var move_width = 0; 
				var a = this.i[inpId].offsetWidth;
				var b = $(this.base).width();
				if(b>a)move_width = b-a;
				else if(a>b)move_width = a-b;
				this.base.style.left = this._getLeft(this.i[inpId])-move_width+"px";
