...
*/
   ...
   if(text!=null && text.indexOf(' ')>0){//xlj add
   		text= text.substring(0,text.indexOf(' '));//xlj add
   }//xlj add
...
*/
      ...
      dhtmlXCombo.prototype._positList = function() {//xlj add
    	  var pos = this.getPosition(this.DOMelem);
    	  var scrollTop=document.documentElement.scrollTop;
    	  if(document.documentElement.clientHeight-(pos[1]-scrollTop)<(24+parseInt(this.DOMlist.style.height))){//剩下的高度<弹出来的那块+行高 ht
    		  this.DOMlist.style.top = pos[1]-this.DOMlist.offsetHeight-parseInt(this.DOMlist.style.height)+"px";
    		  this.DOMlist.style.left = pos[0] + "px";
    	  }else{
    		  this.DOMlist.style.top = pos[1] + this.DOMelem.offsetHeight - 1 + "px";
    		  this.DOMlist.style.left = pos[0] + "px";
      ...
      dhtmlXCombo.prototype._correctSelection = function(){
         ...
         for (var i=0; i<this.optionsArr.length; i++)
            if (!this.optionsArr[i].isHidden()){
            	...
            	if(this.optionsArr[i].text.indexOf(this.getComboText())==0){//xlj add
            		//return this.selectOption(i,true,false);
            		
            		return;//xlj update 20110729  to stop autocompleted action
            	}
           }
         this.unSelectOption();
      }
...
*/
      dhtmlXCombo.prototype._onKeyF = function(e){
         ...
         if (ev.keyCode=="13" || ev.keyCode=="9" ){
         	that.selectOption(that.getSelectedIndex(),false,true);//xlj add
...
*/
      dhtmlXCombo.prototype.selectOption = function(ind,filter,conf){
         ...
         if (filter){
            ...
            //this.setComboText(data[1]);//xlj delete
         }
