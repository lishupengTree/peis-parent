...
(function($) {
	...
	function appendTreeNodes(setting, level, treeNodes, parentNode) {
		...
		for (var i = 0, l = treeNodes.length; i < l; i++) {
			...
			html.push("<a id='", node.tId, IDMark_A,"' draggable='",node[setting.draggable], "' treeNode", IDMark_A," onclick=\"", (node.click || ''),
				...
				"'>",node[setting.nameCol].replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'),"</span>",((node.rightHtml!= null && node.rightHtml.length > 0 && node.rightHtml!= 'null') ? node.rightHtml : ""),"</a><ul id='", node.tId, IDMark_Ul, "' class='", makeUlLineClass(setting, node), "' style='display:", (node.open ? "block": "none"),"'>");
	...
	function zTreePlugin(){
		return {
			...
			updateNodeName : function(treeNode, checkTypeFlag) {
				if (!treeNode) return;
				if (st.checkEvent(this.setting)) {
					var checkObj = $("#" + treeNode.tId + IDMark_Check);
					if (this.setting.checkable) {
