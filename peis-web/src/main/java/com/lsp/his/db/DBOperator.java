package com.lsp.his.db;

import com.lsp.his.utils.DbUtils;
...
public class DBOperator {
    ...
    protected synchronized void initialize(String type) throws Exception {
        if("viewdb".equals(type)){
            Class.forName("oracle.jdbc.driver.OracleDriver");
            String url="jdbc:oracle:thin:@(description=(ADDRESS_LIST =(ADDRESS = (PROTOCOL = TCP)(HOST = 10.1.254.205)(PORT = 1521)))(CONNECT_DATA =(SERVER = DEDICATED)(SERVICE_NAME = orcla)))";
            con = DriverManager.getConnection(url, "his_yhkf", "yhkf123456");
            if (con != null) {
                con.setAutoCommit(false);
            }
        }else if("tjscdb".equals(type)){
            Class.forName("oracle.jdbc.driver.OracleDriver");
            String url="jdbc:oracle:thin:@(description=(ADDRESS_LIST =(ADDRESS = (PROTOCOL = TCP)(HOST = 192.26.3.240)(PORT = 1521)))(CONNECT_DATA =(SERVER = DEDICATED)(SERVICE_NAME = orcl)))";
            con = DriverManager.getConnection(url, "HOSPITAL", "123456");
            if (con != null) {
                con.setAutoCommit(false);
            }
        }else{
    ...
    private ResultSet select(String sql, Object[] params) throws Exception {
        if (showSql) {
            ...
            if (params != null) {
                ...
                if(sql.trim().endsWith("?")==false)//最后一个不是“？”才执行拼接，否则会多拼一段字符串
