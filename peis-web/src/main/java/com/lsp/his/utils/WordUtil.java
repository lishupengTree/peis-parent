...
import com.lsp.his.tables.Bas_worddict;
...
public class WordUtil {
    ...
    public static Map<String, String> GetConvertCode(String str) {
        String py = "";
        String wb = "";
        List<Bas_worddict> bwss = null;
        DBOperator dbe = null;
        try {   //一次性取出表里的数据，再处理
            dbe = new DBOperator();
            String sqlGetWords = "select * from bas_worddict where cnchar in (";
            if ("".equals(str)) {//如果字符串为空
                sqlGetWords += "'')";
            }
            for (int i = 0; i < str.length(); i++) {
                String cnChar = String.valueOf(str.charAt(i));
                sqlGetWords += "'" + cnChar + "'";
                if (i != str.length() - 1) { //不是最后一个
                    sqlGetWords += ",";
                } else {
                    sqlGetWords += ")";
                }
            }
            bwss = dbe.find(sqlGetWords, Bas_worddict.class);
            dbe.commit();
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    public static void main(String[] args) {
        ...
        //System.out.println(getJsonPyWbCode(input));
        System.out.println(trans2PyCode(input));
        System.out.println(trans2WbCode(input));
    }
